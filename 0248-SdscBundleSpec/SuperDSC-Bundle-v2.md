...
- (HBM only) Set `nonUnifiedAllocInHBM_` to control whether the tensor is stored as one unified tensor or as independent per-core tensors:
  - `false` (default) — **unified**: the tensor lives in HBM as a single tensor covering all cores, sized by `N_`. Each core's data is a sub-rectangle of that tensor; all core start addresses are positions within it (common base + per-core offset derived from slice coordinates and layout strides).
  - `true` — **non-unified**: each core's data is stored as its own smaller tensor sized by the per-core `"core"` datastage. These per-core tensors are placed independently in HBM — they need not be contiguous, need not share a common stride, and can be at unrelated addresses. Consequently: (a) `startAddressCoreCorelet_` entries point to each independent tensor rather than to sub-rectangles of a unified one; (b) the allocation coordinates use the same core-fold convention as an LX allocation, since each core holds only its own slice; (c) the per-core tensor layout (`layoutDimOrder_`, stick layout in `primaryDsInfo_`) is described as usual — only the placement in HBM differs. Typically used when the tensor is produced or consumed core-wise and the frontend does not want to materialize a unified copy. Leave `false` for LX allocations.
