...
See [15-json-root-structure.md](2.3-json-root-structure.md) for details.

## SuperDsc Object

See [16-json-superdsc-object.md](2.4-json-superdsc-object.md) for the complete field reference.

## FoldProperty

See [17-json-foldproperty.md](2.5-json-foldproperty.md) for details.

## FoldManager

See [18-json-foldmanager.md](2.6-json-foldmanager.md) for details.
...
| S.No. | Field Name | Purpose / Functionality | How to Fill |
|-------|-----------|------------------------|-------------|
| 8 | `N_` | List of all dimensions across tensors used by ops in all dsc_'s in the sdsc. Contains each dimension's size across all cores. For dimensions that are padded (such as convolution's image dimensions) padding details are also included. See [Padding](1.6-mlir-padding.md). | |
...
| 5 | `padding_` | Padding type applicable for each dimension of the tensor that is padded. | One of NOPAD, LOWERED_PADDED, PADDED_NOZEROPAD, PADDED_WZEROPAD, PADDED_FULLSPAN, and PADDED_FULLSPAN_WUNNEEDED. See [Padding](1.6-mlir-padding.md) for more details. |
...
| 8 | `isStartAddrSymbolic_` | 0 or 1 indicating whether the start addresses specified in `data_` field of `startAddressCoreCorelet_` are symbolic. | See [Folding](1.7-mlir-folding.md) |
| 8 | `startAddressCoreCorelet_` | Start address of a tensor per core. | See [Folding](1.7-mlir-folding.md) |
| 9 | `coordinates_` | Tensor coordinates per dimension. | See [Folding](1.7-mlir-folding.md) |
